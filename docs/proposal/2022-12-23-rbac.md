...
- [Proposal](#proposal)
  ...
  * [Executor isolation](#executor-isolation)
- [Reference](#reference)
...
To ensure no plugin has access to the sensitive configuration data, like Slack or Discord tokens, or other temporary Kubeconfigs, we will modify the plugin manager to run command with in an isolated environment in the following way. For each plugin execution:

1. Create a temporary directory.
1. Create a temporary Kubeconfig file in the directory.
1. Symlink the plugin binary to the temporary directory and all other required dependencies (e.g. `helm` or `kubectl` binary).
...
One thing to note is that for such Kubeconfig we still use the same token as the Botkube installation. That means, user needs to trust each plugin before execution, as it can behave maliciously, and e.g. impersonate a `cluster-admin` with a modified Kubeconfig.
...
Still, a harmful plugin can use any of the group names / users defined in the ClusterRole (if it somehow guesses it) and we cannot prevent this, if we pass the token with ability to impersonate.
