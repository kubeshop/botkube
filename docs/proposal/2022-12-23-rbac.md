...
To ensure no plugin has access to the sensitive configuration data, we will modify the plugin manager to run command with different user and/or group id. It would be recommended to use Chroot, to make sure plugins cannot access other temporary Kubeconfigs. That will require root access or some root-less workarounds, and possibly copying/symlinking some system directories to ensure the GRPC server can run.
