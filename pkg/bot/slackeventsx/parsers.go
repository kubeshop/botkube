...
func parseInnerEvent(e *EventsAPICallbackEvent) (EventsAPIEvent, error) {
	...
	innerBytes, _ := json.Marshal(*rawInnerJSON)
	err = json.Unmarshal(innerBytes, &inner)
	dataBytes, err := json.Marshal(inner["data"])
	err = json.Unmarshal(dataBytes, recvEvent)
	if err != nil {
		return EventsAPIEvent{
			e.Token,
			e.TeamID,
